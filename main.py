...
async def setprefix(ctx, *, newprefix: str):
  ...
  if len(newprefix) > 2:
    embed_message = discord.Embed(
      color=(0xffffff),
      description=(
        f"> <:caution:1096425475590598836>  {ctx.author.mention}: Prefix is too large"
      ),
    )
    ...
    embed_message = discord.Embed(
      color=(0xffffff),
      description=(
        f"> <:check:1096543207837421648>  {ctx.author.mention}: Guild prefix changed to `{newprefix}`"
      ),
    )
    await ctx.reply(embed=embed_message, mention_author=False)
...
async def upd(ctx):
  embed_loading = discord.Embed(title="",
                                description=f"""> **Restarting all cogs**
    ...
    *-* `owner.py` <a:loadingcircle:1097220809359110245>""",
                                color=0xffffff)
  embed_done = discord.Embed(title="",
                             description=f"""> **Restarting complete**
    
    **Cogs**
    *-* `ping.py` <:check:1097220806624432168>
    *-* `moderation.py` <:check:1097220806624432168>
    *-* `main.py` <:check:1097220806624432168>
    *-* `owner.py` <:check:1097220806624432168>""",
                             color=0xffffff)
